...
// 3D 회전의 표현과 변환을 다루기 위해 사용

#include <sphericalsfm/so3.h>

namespace sphericalsfm {

    Eigen::Matrix3d skew3(const Eigen::Vector3d &v ) //skew matrix form으로 바꿔주는 역할 
    {
        Eigen::Matrix3d s;
        s <<
        0, -v[2], v[1],
    ...
    Eigen::Matrix3d so3exp(const Eigen::Vector3d &r) // rotation vector를 rotation matrix로 변환 
    {
    ...
    Eigen::Vector3d so3ln(const Eigen::Matrix3d &R) // rotation matrix를 3d vector로 변환해줌 
    {
